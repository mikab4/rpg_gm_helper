...
def build_entity_type_compatibility_report(db_session: Session) -> EntityTypeCompatibilityReport:
    ...
    for legacy_type, entity_id, entity_name, campaign_id, campaign_name in example_rows:
        if len(examples_by_type[legacy_type]) >= EXAMPLE_LIMIT_PER_TYPE:
            continue

        examples_by_type[legacy_type].append(
            LegacyEntityTypeExample(
                entity_id=entity_id,
    ...
    issues = [
        LegacyEntityTypeIssue(
            ...
            count=issue_count,
            example_entities=examples_by_type[legacy_type],
        )
        for legacy_type, issue_count in legacy_type_rows
    ]
...
def migrate_entity_types(
    ...
    unresolved_legacy_types = {issue.legacy_type for issue in compatibility_report.issues}
    mappings_by_legacy_type = {
        mapping.legacy_type.strip().lower(): mapping.target_type.value
